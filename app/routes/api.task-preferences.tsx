...
export async function action({ request }: { request: Request }) {
  ...
  if (action === "set_multiple_preferences") {
    ...
    if (!personName || !preferencesJson) {
      console.log("Missing fields for multiple preferences:", {
        personName,
        preferencesJson,
      });
      return new Response(
    ...
    try {
      ...
      for (const [taskId, preference] of Object.entries(preferences)) {
        if (preference) {
          // Save all valid preferences, including 'indiferente'
  ...
  if (!taskId || !personName || !preference) {
    console.log("Missing fields for single preference:", {
      taskId,
      personName,
      preference,
    });
